import java.util.LinkedList;

public class linkedList {

    private Node head;
    private Node tail;
    private int length;

    class Node{
    String date;
    Node next;

       Node(String date) {
            this.date = date;
        }

    }

    public linkedList(String date){
        length = 1;
        Node nextNode = new Node(date);
        head = nextNode;
        tail = nextNode;
    }
    public void gethead() {
        if (this.head == null){
            System.out.println("Lista vazia/");
        }else {
            System.out.println("Head: " + head.date);
        }
    }

    public void gettail() {
        if (this.tail == null){
            System.out.println("Lista vazia/");
        }else {
            System.out.println("Tail: " + tail.date);
        }
    }
    public void getlength() {
        System.out.println("Length:" + this.length);
    }

    public void makeEmpty(){
        head = null;
        tail = null;
        length = 0;
    }

    public  void print () {
        Node temp = this.head;
        while (temp != null){
            System.out.println(temp.date);
            temp = temp.next;
        }
    }

    public void append(String data){
        Node nextNode = new Node(data);
        if (length == 0){
            head = nextNode;
            tail = nextNode;
        }else {
            tail.next = nextNode;
            tail = nextNode;
        }
    }

    public Node removeLast(){
        if (length ==0) return null;
        Node pre = head;
        Node temp = null;
        while (pre.next != tail){
            pre = pre.next;
        }
        temp = tail;
        tail = pre;
        tail.next = null;

        length--;
        if (length ==0){
            head = null;
            tail = null;
        }

        return  temp;
    }

        public  void prepand(String data){
        Node nextNode = new Node(data);
        if (length == 0){
            head = nextNode;
            tail = nextNode;
        }else {
            nextNode.next = head;
            head = nextNode;
        }
        length++;
        }

        public  Node removeFirst(){
        if (length == 0) return null;
            Node temp = head;
            head = head.next;
            temp.next = null;
            length--;
            if (length == 0){
                head = null;
                tail = null;
            }
            return temp;
        }

        public Node get (int index){
        if (index < 0 || index >= length) return null;
        Node temp = head;
        for (int i = 0; i < index; i++){
            temp = temp.next;
        }
        return  temp;
        }
    public boolean insert(int index,String data){
        if (index < 0 || index >= length) return false;
        if (index == 0){
            prepand(data);
            return true;
        }
        if (index == length){
            append(data);
            return true;
        }
        Node nextNode = new Node(data);
        Node temp = get(index);
        nextNode.next = temp.next;
        length++;
        return true;

    }

    public  boolean set(int index,String data){
        Node temp = get(index);
        if (temp != null){
            temp.date = data;
            return true;
        }
        return true;
    }

    public Node remove (int index){
        if (index < 0 || index >= length)return null;
        if (index == 0)return removeFirst();
        if (index == length)return removeLast();

        Node prev = get(index-1);
        Node temp = prev.next;

        prev.next =temp.next;
        temp.next = null;
        length--;
        return temp;
    }


    public static void main(String[] args) {
        linkedList list = new linkedList("elementos 1");
        list.append("elemento 2");
        list.append("elemento 3");
        list.prepand("elemento 0");
        list.remove(2);
        //list.insert(3,"elemento 2.5");
      //  System.out.println(list.get(2).date);
       // System.out.println(list.removeFirst().date);
        list.print();
       list.set(1,"elemento 0.5");
        linkedList list1 = list;
        list1.print();

    }

}
